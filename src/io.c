...
bimageSaveJPG(bimage *im, const char *filename, int quality) {
  ...
  return stbi_write_jpg(filename, im->width, im->height,
                        bimageTypeChannels(im->type), im->data, quality) == 0
             ? BIMAGE_ERR
             : BIMAGE_OK;
}
...
bimageSave(bimage *im, const char *filename) {
  ...
  if (strncasecmp(x, "png", 3) == 0 && bimageTypeDepth(im->type) == BIMAGE_U8) {
    return stbi_write_png(filename, im->width, im->height,
                          bimageTypeChannels(im->type), im->data,
                          im->width * bimageTypeChannels(im->type)) == 0
               ? BIMAGE_ERR
               : BIMAGE_OK;
  } else if (strncasecmp(x, "hdr", 3) == 0 &&
             bimageTypeDepth(im->type) == BIMAGE_F32) {
    return stbi_write_hdr(filename, im->width, im->height,
                          bimageTypeChannels(im->type), im->data) == 0
               ? BIMAGE_ERR
               : BIMAGE_OK;

...
{
    bimagePixel px;
    bimagePixelInit(&px, src->data.f[0], src->data.f[1], src->data.f[2], src->data.f[3]);
    return px;
}
...
{
    return bimagePixelInit(px, 0, 0, 0, 0);
}
...
bimagePixelClamp(bimagePixel *px)
{
    int i;
    for(i = 0; i < 4; i++){
        px->data.f[i] =  px->data.f[i] < 0.0 ? 0.0 : px->data.f[i] > 1.0 ? 1.0 : px->data.f[i];
    }

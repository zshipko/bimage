...
static BIMAGE_TYPE types[] = {
    BIMAGE_GRAY | BIMAGE_U8,  BIMAGE_GRAY | BIMAGE_U16,
    BIMAGE_GRAY | BIMAGE_U32, BIMAGE_GRAY | BIMAGE_F32,
    BIMAGE_GRAY | BIMAGE_C32, BIMAGE_GRAY | BIMAGE_F64,
    BIMAGE_RGB | BIMAGE_U8,   BIMAGE_RGB | BIMAGE_U16,
    BIMAGE_RGB | BIMAGE_U32,  BIMAGE_RGB | BIMAGE_F32,
    BIMAGE_RGB | BIMAGE_C32,  BIMAGE_RGB | BIMAGE_F64,
    BIMAGE_RGBA | BIMAGE_U8,  BIMAGE_RGBA | BIMAGE_U16,
    BIMAGE_RGBA | BIMAGE_U32, BIMAGE_RGBA | BIMAGE_F32,
    BIMAGE_RGBA | BIMAGE_C32, BIMAGE_RGBA | BIMAGE_F64,
};
...
START_TEST(test_bimageConsume) {
  ...
  ck_assert(px.data.f[0] == p.data.f[0] && px.data.f[1] == p.data.f[1] &&
            px.data.f[2] == p.data.f[2] && px.data.f[3] == p.data.f[3]);
  bimageRelease(a);
}
END_TEST
...
START_TEST(test_bimageAdd) {
  ...
  ck_assert(d.data.f[0] == 1 && d.data.f[1] == 0 && d.data.f[2] == 0 &&
            d.data.f[3] == 1);
  bimageRelease(im);
  bimageRelease(im2);
}
...
START_TEST(test_bimageCopyTo) {
  ...
  for (i = 0; i < num_types; i++) {
    ...
    ck_assert(a.data.f[0] == b.data.f[0] && a.data.f[1] == b.data.f[1] &&
              a.data.f[2] == b.data.f[2]);
...
START_TEST(test_bimageEachPixel) {
  ...
  for (size_t i = 0; i < im->width * im->height * bimageTypeChannels(im->type);
       i++) {
...
START_TEST(test_bimageDisk) {
  ...
  BENCH_STOP(disk1);
  ck_assert((im->width == 500 && im->height == 500 &&
             (im->type == (BIMAGE_F32 | 3))));

  bimageGetPixelUnsafe(im, 25, 25, &px2);
  ck_assert((px2.data.f[0] == px.data.f[0] && px2.data.f[1] == px.data.f[1] &&
             px2.data.f[2] == px.data.f[2]));

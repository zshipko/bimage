...
static cv::Scalar bimagePixelToScalar(bimagePixel *px) {
  return cv::Scalar(px->data.f[0], px->data.f[1], px->data.f[2], px->data.f[3]);
}

static bimagePixel bimagePixelFromScalar(cv::Scalar src) {
  return bimagePixelCreate(src[0], src[1], src[2], src[3]);
}

static int bimageTypeToMatType(BIMAGE_TYPE t) {
...
static int bimageTypeFromMatType(int channels, int depth) {
  switch (depth) {
  case CV_8U:
...
BIMAGE_CV_API
bimagePixel bimageVariance(bimage *_image) {
...
BIMAGE_CV_API
bimagePixel bimageMean(bimage *_image) {
  cv::Mat image = bimageToMat(_image);
  cv::Scalar mean, stddev;
...
BIMAGE_CV_API
bimage *bimageMatchTemplate(bimage *_dest, bimage *_image, bimage *_templ,
                            int method, bimage *_mask) {
